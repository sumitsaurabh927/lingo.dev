  ...
  private configureGit() {
    ...
    if (!processOwnCommits) {
      ...
      if (authorOfLastCommit === currentAuthor) {
        this.ora.warn(
          `The last commit was already made by ${currentAuthor}, so this run will be skipped, as running again would have no effect. See docs: https://lingo.dev/cli/ci-action/overview`,
        );
        return false;
      }
    }
